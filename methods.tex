...
temporal window of the previous 14 days as conditioning data. The
...
feasibility. Spatial propagation occurs only horizontally within each
depth level, while vertical consistency is implicitly maintained
through the depth-specific variogram structures derived from the 3D
deterministic model.
...
Although the underlying deterministic model (CMS model) is fully
three-dimensional and the statistical solution also provides
predictions across multiple depth layers (as discussed in the previous
subsection), in this work, the sampling algorithm operates on a 2D
...
horizontal slices of uncertainty between the surface and 40 m depth, a
...
surface and upper thermocline. Averaging across this depth range,
...
2D planning approximation and enabling the acquisition of
high-vertical-resolution profiles at all visited locations. This
strategy also aligns with the dominant physical and biogeochemical
processes occurring within the mixed layer, where air–sea
interactions, nutrient entrainment, and local thermal structure exert
primary control on short-term dynamics.
