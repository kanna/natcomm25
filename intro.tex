...
are characterized by high variabity, intricate topographies, and
...
% generating improved forecasts for subsequent adaptive planning 
as a virtuous cycle. Doing so closes a critical feedback loop between
models and observations; model prediction generates a forecast and
associated uncertainty field while AUV based adaptive sampling targets
areas of maximum predicted uncertainty. Data assimilation from these
high uncertainty regions are then incorporated into the model such
that new predictions benefit from improved in-situ data, closing the
loop.  Such a \emph{sample-assimilate-predict-direct} data cycle
approach promises to significantly enhance the skill of ocean
forecasts, especially in the complex coastal environment where
processes occur over a wide range of scales. % It also has the potential
% to improve operational efficiency by optimizing the deployment of
% expensive and resource-constrained observational assets.
Additionally, sampling resolution with AUVs vastly outweighs data
obtained from traditional ship-based approaches. Multiple AUVs
sampling simultaneously as we do here, also makes a dent in spatial as
well as temporal coverage of such dynamic environments at far moderate
costs. Further, AUVs can provide near real-time data allowing for
their rapid re-tasking and potentially placement in spatially
separated regions. % Operating multiple vehicles in coastal regions
% while requiring flexibility in operations to avoid marine traffic,
% nets and crab-pots, also allows data collection in complex operating
% environments.
...
(Fig. \ref{fig:block-diag}).% \proj provided
% the experimental setting and operational assets to demonstrate how
% model-based uncertainty fields can drive adaptive sampling missions
% aimed at improving ocean model predictive skills to enhance ocean
% forecasts in highly dynamic coastal environments
The experimental results obtained within \proj serve to illustrate and
validate the proposed approach, highlighting the practical benefits
and challenges of real-world adaptive ocean exploration. \proj itself
is part of a larger concept for multi-domain sensing, observation and
ocean exploration, which couples ocean models, autonomous robotics,
Artificial Intelligence and Machine Learning coupled with a small
satellite constellation \cite{rajan21}.


This manuscript is organized as follows. Section \ref{sec:intro}
provides an introduction to the problem we're tackling providing a
