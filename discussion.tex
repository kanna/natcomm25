...
field. Fig. \ref{fig:iqd3D} and \ref{fig:iqd3D_zoom} depicts the interquartile distance (IQD)
...
\begin{figure}[!]
  ...
  \caption{{\subref{fig:iqd3D}} Trajectory of XP5 AUV, during the 30\textsuperscript{th}
    October mission in the \naz Canyon region. The color shading represents the interquartile range (IQD) of temperature values recorded within each model grid cell along the vehicle’s path. The grid illustrates the horizontal resolution of the statistical model. Results are shown for three representative depth layers: near-surface, mid-depth (20 m), and 40 m, capturing distinct vertical regimes. Higher IQD values indicate regions of higher thermal variability within the model grid. {\subref{fig:iqd3D_zoom}} Close-up view of the mid-depth layer (~20 m) showing individual model grid cells intersected by the XP5 trajectory (red dots), revealing marked fine-scale fluctuations occurring inside single grid elements. This zoomed perspective illustrates how AUV sampling captures subgrid-scale thermal structure that the model’s horizontal resolution does not resolve.}
\end{figure}
