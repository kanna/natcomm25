...
\item \textbf{Targeted Sample Planning}: Here we define areas of high reward, those associated with larger uncertainty predictions. Using the uncertainty map
...
impractical for use with high spatial resolution and in near-real-time mission planning \kcomment{Need
  a citation here is a must indicating what it takes to model using
  high res. numerical models. See my comment in the discussion section
  related to subgrid variability to make a strong case for the model
  we actually used.}. While similar assimilation cycles could, in
...
sequential simulation as a computationally efficient surrogate for short-term spatial predictions of ocean variables together
...
calibrated model and prediction time length. If the calibrated model is unable to capture the
