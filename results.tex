...
explore the temperature variability predicted by the statistical
model, on 31\textsuperscript{st} October, XP2 followed cross-shore
...
ground-truth dataset. These observations are thus used as a reference
for evaluating the forecast cases \textbf{C1–C4} and \textbf{D1–D4}.
...
adaptive sampling and data assimilation framework.  The pronounced
...
of how AUV-based targeted observations can improve the predictive
skill of the statistical model in a real-world coastal scenario.
...
of the surface layer down to approximately 40 m depth, with
...
to 1.33$^{\circ}$C at 40 m, with a mean RMSE of about
...
the upper layers but diverges below 20 m. After assimilation, the
...
5–-20 m layers, coinciding with the strongest vertical temperature
...
Figs. \ref{fig:rms_c} and \ref{fig:rms_d}, correspond to the phase of
the experiment in which multiple assimilation scenarios were
tested. The \textbf{C}-series uses CMS data up to
30\textsuperscript{th} October, whereas the \textbf{D}-series starts
from CMS data up to 29\textsuperscript{th} October, representing a
less favorable statistical initial state. In both cases, the
assimilation scenarios (\textbf{C1-–C4} and \textbf{D1-–D4})
correspond to the incorporation of AUV data collected during the
previous days.
...
the fully assimilated cases (\textbf{C4} and \textbf{D4}),
particularly in the upper 30 m, where the assimilation of prior AUV
data significantly improves the representation of the temperature
structure and short-term thermal evolution probably associated with
the upwelling–relaxation transition.
...
\begin{figure}
  ...
  initialized using CMS data available up to 30 October, and the
  assimilation steps progressively include AUV data collected on
  previous days. Results are presented for each vehicle individually
  and for the combined dataset (all AUVs).]
  {\label{fig:rms_c}\includegraphics[width=1\linewidth]{fig/rms_c.png}}

  \subfigure[RMSE (in $^{\circ}$C) between statistical model forecasts
  and in situ AUV temperature measurements for 31 October, scenarios
  \textbf{D}, \textbf{D1-–D4}. The forecasts were initialized using
  CMS data available up to 30\textsuperscript{th} October, and the
  assimilation steps progressively include AUV data collected on
  previous days. Results are presented for each vehicle individually
  and for the combined dataset (all AUVs).]
...
days (\textbf{C4} and \textbf{D4}). This comparison highlights the
point-wise behaviour of the statistical model under minimum and
maximum assimilation influence.
...
\textbf{D}), the scatter plots exhibit a wide spread of points around
the reference line, indicating substantial discrepancies between the
model predictions and the observed AUV temperatures. These deviations
are particularly evident in \kcomment{...,????} where surface
temperatures tend to be overestimated. This dispersion reflects the
model’s limited ability to accurately reproduce the vertical thermal
structure when it relies solely on CMS data without incorporating
recent in situ information.

In contrast, the fully assimilated scenarios (\textbf{C4} and
\textbf{D4}) show a much tighter clustering of points around
reference, especially within the upper layers, where the previous AUV
missions had provided dense and temporally relevant observations. This
...
\begin{figure}[!]
  ...
  \caption{Comparison between in situ AUV observations
    ...
    yellow), while deeper layers (>30 m) are progressively represented
    in cooler shades (green to blue). The dashed red line provides a
    reference for perfect agreement.  \rmcomment{!not the final
      figure. need to correct and round values to 1 decimal place!}}
  \label{fig:scatter}
\end{figure}
