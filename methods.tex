...
sensors including a CTD (Fig. \ref{fig:lauvs}) while some have in
addition fluorometers, turbidity, Oxygen, DVL/ADCP sensors. All
vehicles come equiped with WiFi and Iridium satellite communications,
...
extensive suite of mature and advanced mission planning and
...
\kcomment{Need a paragraph, potentially with a figure on the
...
setting. While the AUVs had a range of sensors, the model's workflow
was focused only towards temperature data. As a consequence this work
is univariate in showing the impact of assimilated model-driven
coastal exploration. We believe that results in Sec \ref{sec:results}
are general enough to apply to other critical oceanographic variables
which can be modeled similarly.
...
uncertainty field based on the Copernicus Marine Serice (CMS)
...
the planned routes. The data collected within the previous day were
subsequently assimilated into the statistical model to produce updated
forecasts, enabling comparisons between forecasts with and without
data assimilation.

The experiment involved three AUVs XP2, XP3 and XP5, which alternated
daily in performing targeted sampling missions over the \naz Canyon
region.
