...
\item \textbf{Targeted Sample Planning}: Here we define areas of high
  reward, those associated with larger uncertainty predictions. Using
  the uncertainty map $M_{\sigma_{\hat{\theta}}}(x, y)$ as input, a
  targeted sampling algorithm determines the set of trajectories for a
  fleet of $N$ AUVs for the next operational cycle. The goal is to
  maximize the accumulated uncertainty sampled along the vehicle
  paths, while satisfying vehicle-specific constraints. The planned
...
to demonstrate adaptive sampling, here the trajectories are defined by
human-in-the-loop, with adaptation focused on deployment based on
$M_{\sigma_{\hat{\theta}}}(x, y)$. Loop closure in this work, refers
to the \emph{sample-assimilate-predict-direct} process as shown in
...
mission planning \kcomment{Need a citation here is a must indicating
  what it takes to model using high res. numerical models. See my
  comment in the discussion section related to subgrid variability to
  make a strong case for the model we actually used.}. While similar
assimilation cycles could, in principle, be implemented directly
within deterministic numerical models, this would require either
substantially higher computational resources or larger temporal
horizons.
...
geostatistical approach will not be captured.
...
\cite{soares2001direct}\footnote{Defined by kriging estimates and
  variances, which are conditioned to existing direct measurements and a
...
\cite{CMEMS2017}. For each depth layer of the deterministic model, geostatistical simulations are performed independently using a moving temporal window of the previous 14 days as conditioning data. The window length was selected through comparison with the deterministic model’s short-term evolution under similar seasonal conditions, striking a balance between forecast stability and computational feasibility. Spatial propagation occurs only horizontally within each depth level, while vertical consistency is implicitly maintained through the depth-specific variogram structures derived from the 3D deterministic model.

%For each depth of the numerical ocean model,
%geostatistical simulations are carried out independently, using a moving temporal window of fourteen previous days as conditioning data (i.e., experimental data) to predict the subsequent day. The size of the temporal window was achieved by trial-and-error using a calibrated numerical model for the same region and under the same season, and comparing the geostatistical predictions against the response of the numerical ocean model. This sliding-window strategy strikes a balance between forecast skill and computational feasibility, and can be adapted according to the complexity of the oceanographic conditions. 
\kcomment{AN EXPLANATION OF THE METHOD THAT PROCEEDS BY
  DEPTHS MAY BE USEFUL. Is PROPAGRATION DONE ONLY IN THE HORIZONTAL
  DOMAINS?}, \rmcomment{corrected}
...
\begin{itemize}
    \item WE HAVE NOT DISCUSSED THE FACT THAT THE INPUT TO THE SAMPLING ALGORITHM IS JUST AM HORIZONTAL SLICE OF THE ERROR PREDICTION \rmcomment{it is not. the sampling algorithm is based on an average of the horizontal slices between surface and 40 m - i introduced some text in next section}
    \item but this can be somewhat compensated in the vertical domain by the YOYO \rmcomment{done in The Sampling Algorithm section}
    \item but we may need to explain how have we selected the depth to be used in the sampling algorithm \rmcomment{done in The Sampling Algorithm section}
    \item discussion about the mixed layer depth as suggested by Ajit? \rmcomment{partially done in The Sampling Algorithm section}
\end{itemize}
}
...
Although the underlying deterministic model (CMS model) is fully three-dimensional and the statistical solution also provides predictions across multiple depth layers (as discussed in the previous subsection), in this work, the sampling algorithm operates on a 2D horizontal field derived from the 3D uncertainty structure of the statistical solution. This field is obtained by averaging the horizontal slices of uncertainty between the surface and 40 m depth, a depth interval chosen to represent the upper mixed layer in the study region. Within this layer, turbulent mixing typically maintains relatively weak vertical gradients, and most of the temperature variability relevant for mission planning is concentrated near the surface and upper thermocline. Averaging across this depth range, therefore, yields a stable and physically meaningful 2D representation of the uncertainty field.

Although the trajectory planning itself is carried out in two dimensions, the yo-yo motion executed by the AUVs along each transect naturally restores vertical sampling capability, compensating for the 2D planning approximation and enabling the acquisition of high-vertical-resolution profiles at all visited locations. This strategy also aligns with the dominant physical and biogeochemical processes occurring within the mixed layer, where air–sea interactions, nutrient entrainment, and local thermal structure exert primary control on short-term dynamics.
...
\begin{figure}
  \centering
  \includegraphics[scale=0.15]{fig/scheme.png}
...
scenario solutions contribute to the final solution set. These
scenarios, labelled \textbf{A} through \textbf{D} (and their
corresponding assimilation variants) are summarised in Table
\ref{table:scenarios}. The table provides a compact, colour-coded
overview of all configurations, including forecast dates, prior CMS
fields, and the specific AUV datasets assimilated at each stage,
serving as a quick reference for the results presented in the
following sections.
...
mission executed by AUV XP2 (Fig. \ref{fig:scheme}). The resulting
temperature data were later assimilated offline to produce an updated
statistical model solution (\textbf{B1}) for 30\textsuperscript{th}
...
skill. Temperature measurements from XP3 and XP5 on
30\textsuperscript{st} October were assimilated offline to generate
scenarios \textbf{C1}-–\textbf{C4}, complementing the non-assimilated
reference case \textbf{C}. These cases progressively integrate
different subsets of AUV observations, ranging from XP2-only data
(\textbf{C1}) to the full multi-vehicle dataset from
29\textsuperscript{st} to 30\textsuperscript{st} October
(\textbf{C4}).
...
\begin{table}[]
\caption{Summary of scenarios and assimilation configurations.}
