...
While previous efforts have concentrated on a form OF? on embedded
...
variability of ocean properties. However, conventional DETERMINIST numerical ocean
models are computationally intensive, and their runtime makes them
impractical for use in near-real-time mission planning. 

BEGIN ELIMINATE

While similar assimilation cycles could, in
...
variability of local ocean dynamics, based on calibrated deterministic dynamic ocean
...
the work approach. However, with this type of surrogate models we are not explicitly considering the full-physics of the system and the quality of the predictions directly depend on the accuracy of the calibrated model. If the calibrated model is unable to capture the true nature of the system the geostatistical predictions will also be inaccurate.  Also, if there is a new oceanographic event that is not present in the calibrated model, the geostatistical approach will ne be able to capture it. 
...
\kcomment{Ana, we need a paragraph, potentially with a figure on the
  ...
  so, how so and with what frequency?} \rmcomment{only AUV data was assimilated. the paragraph was included in the previous section Model Forecast, Uncertainty Projection and Data Assimilation (I also changed the name of the section to accomodate the assimlation description}
