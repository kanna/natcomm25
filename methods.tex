...
\item[Model Forecast and Uncertainty Projection]: A numerical ocean
  ...
  representing respectively, the predicted state and its uncertainty
  over a predefined grid covering the study area.

\item \textbf{Targeted Sampling}: We define areas of high reward,
  those associated with larger uncertainty predictions. Using the
  ...
  maximize the accumulated uncertainty sampled along the vehicles
  trajectories, while satisfying vehicle-specific constraints. 
...
\item \textbf{Data Collection and Assimilation}: Throughout the
  ...
  the collected measurements are assimilated into the numerical model
  using an appropriate data assimilation scheme. This updated model
  state serves as the new initial condition for the next forecasting
...
the trajectories in this work are defined manually, with adaptation
focused on deployment based on $M_{\sigma_{\hat{\theta}}}(x, y)$. Loop
closure in this work, therefore, refers to the
...
mission planning \cite{agarwal21} \kcomment{Need a citation here is a
  must indicating what it takes to model using high res. numerical
  models. I've put one in for now, but not exactly the citation I was
  hoping for.}. While similar assimilation cycles could, in principle,
...
\cite{soares2001direct}\footnote{Defined by kriging estimates and
  ...
  a spatial covariance matrix.}, where each simulated value is drawn
...
a priori information. The ensemble of realizations thus provides an
updated ocean forecast and a quantitative measure of
uncertainty.
...
short-term dynamics \kcomment{Need a citation}.
