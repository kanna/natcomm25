...
Understanding the impact of targeted AUV observations on short-term ocean forecasts requires examining not only overall model–data agreement but also how assimilation effects vary across vehicles and across distinct dynamical environments. While the general trend indicates that assimilation improves model skill, vehicle-specific results reveal important differences tied to the spatial distribution of sampling relative to dynamically coherent regions. 
...
A broader interpretation of this result is provided by the CMS fields shown in Figure \ref{fig:north_south}, which compare temperature distributions in the northern and southern sectors during the days preceding and during the \proj experiment. Before 26\textsuperscript{th} October, although mean temperatures differ between the two regions, their variability is highly consistent, indicating that the system was evolving coherently across the canyon. After 26\textsuperscript{th} October, however, the temperature evolution in the two regions diverges substantially, with the CMS model showing increasingly decorrelated variability between North and South. This loss of coherence coincides with the onset of the upwelling–relaxation transition and highlights a shift toward slightly distinct dynamical regimes on either side of the canyon. This transition can explain why assimilation of southern data—collected under a single dynamical regime—fails to improve and, in fact, degrades forecasts.

This spatial decoupling between sampling and assimilated information also highlights the potential value of trajectory-planning strategies that explicitly seek to maximise class separation among distinct dynamical regimes, as proposed by \cite{fossum19b} using hierarchical clustering methods. Such approaches, based on criteria such as expected reduction of variance, mutual information, or entropy, could help identify routes that better discriminate between competing oceanographic states and mitigate situations where assimilation may propagate misleading information if relying solely on uncertainty fields.
\rmcomment{we can talk here about the clustering/ allocation by areas
  ...
  future studies YES. DONE}

\begin{figure}[!]
  \centering
  \includegraphics[scale=0.2]{fig/north_south.png}
  \caption{Temperature distributions from the CMS model for the northern and southern sectors of the study area between 18\textsuperscript{th}–31\textsuperscript{th} October. The division between regions follows the boundary shown in the onset map (top right). Only grid points shallower than 200 m were retained to avoid including the canyon interface. Boxplots show the spatial distribution of CMS temperature within each region, capturing both the median and the spread of variability.}
...
the presence of subgrid-scale dynamics. Such unresolved variability is a well-known source of representativeness error in data assimilation systems: numerical forecasts and statistical surrogates inevitably smooth or filter variability occurring below their grid scale, so observations that resolve submesoscale or turbulent features will often disagree locally with grid-averaged model fields \cite{oke2008representation,janjic2018representation}. These considerations underscore that high-resolution observations from AUVs should not be interpreted solely through coarse model fields; instead, subgrid variability itself provides valuable information about local forecast uncertainty and should be explicitly accounted for when assimilating observations into coarser-resolution predictive systems.\kcomment{We need a citation or something to buttress this claim.}\rmcomment{review done}
...
\kcomment{Of temperature?}\rmcomment{no. measurements per cell} between 19 and 26. These statistics
...
mixing events. Because AUVs sample continuously along their trajectories, they resolve spatial and temporal scales that often fall between CTD stations in traditional ship-based surveys, thereby capturing subgrid processes that remain unresolved in numerical models. \kcomment{What is the evidence for
  ...
  takeaway from this experiment, we perhaps not mentioned it.} \rmcomment{review done}
...
assimilation frameworks enhances the model’s ability to represent
...
forecasts. \kcomment{But did we not assimilate \emph{all} the AUV
  ...
  the ``could enhance'' as against ``it did enhance''?} \rmcomment{We did not assimilate \textit{all} individual measurements; instead, we assimilated grid-averaged observations within each model cell.}
