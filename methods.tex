...
variability of ocean properties. However, conventional DETERMINIST
numerical ocean models are computationally intensive, and their
runtime makes them impractical for use in near-real-time mission
planning \kcomment{Need a citation here}. While similar assimilation
cycles could, in principle, be implemented directly within
deterministic numerical models, this would require either
...
predictions of OCEAN VARIABLES INSTEAD OF TEMPERATURE? WE HAVE NOT DISCUSSED TEMPERATURE YET ocean temperature together with estimates of spatial
uncertainty \cite{deutsch1992}. This approach captures the essential
variability of local ocean dynamics, based on calibrated deterministic
dynamic ocean models, at a fraction of the computational cost of full
DETERMINISTIC ? numerical models, while remaining flexible enough to QUICKLY assimilate new in
situ observations ERASE quickly \cite{Duarte2025} and CONSISTENT? coherent with the
scope of the work approach. However, with this type of surrogate
models we are not explicitly considering the full-physics of the
IT IT NOT CLEAR WHAT IS SYSTEM HERE system and the quality of the predictions directly dependS on the
accuracy of the calibrated model. If the calibrated model is unable to
capture the true nature of the system the geostatistical predictions
will also be inaccurate.  Also, if there is a new oceanographic event
that is not present in the calibrated model, the geostatistical
approach will NOT ne be able to capture it.
...
PLEASE READ THESE ITEMS; THIS WAS INSERTED WED AT 1PM
\begin{itemize}
...
UNDERSTAND WHAT WAS REALLY DONE AND HOW IT WAS DONE

SOMETHING LIKE

The experiment had a small human and support vessels footprint, thus demonstrating that a significant cost reduction in comparison to traditional ship-based methods in addition to higher spatial and temporal resolutions. Operations were conducted non-stop by pairs of operators working 6 hour shift from command and control centers based in Porto, as well as in Nazaré. The intervention of 2 small boats was limited to the launch and recovery of AUVs, that operated not attended for up to 50h+. 

OK????

Challenging meteorological and ocean conditions constrained the NUMbER OF CONSECUTIVE iterations of the
\emph{sample-assimilate-predict-direct} loop to a three-day sequence
of experiments (29-–31 October, 2025)
(Fig. \ref{fig:scheme}). 

SWAPPED THE POSITIONS OF THE NEXT TWO PARAGRAPHS - PLEASE READ THE FIRST ONE CAREFULLY 
FOR THE SAKE OF CLARITY, the PRESENTATION OF THE workflow and 
assimilation loop IS focused only ON temperature data. As a
...
oceanographic variables which would be modeled similarly.

The analysis in this work aims to quantify
...
THE FOLLOWING PARAGRAPH DOES NOT READ WELL; 

STARTS BY THE GENERATION OF THE MODEL ???? IS THIS JUST FOR THE FIRST DAY? IF SO NEEDS TO BE EXPLAINED

FOLLOWED BY THE EXECUTION OF THE ALGORITHM 

AND WHERE IS THE ASSIMILATION PRECEEDING THE ADAPTIVE SAMPLING ALGORITHM?
...
execution of the target sampling algorithm to plan the NEXT? next-day
...
SAME PROBLEM WITH THE CAPTION OF THE NEXT FIGURE


WHAT IS TARGET SAMPLING IN THE FIGURE? THIS IS NOT SAMPLING, THIS IS EXECUTING THE TOUR GENERATION ALGORITHM, RIGHT?

LIKEWISE WHAT IS AUV DATA ACQUISITION? DATA AQUISITION HAS BEEN PERFORMED DURING SAMPLING

THIS MAY BE DATA INGESTION AND QC
\begin{figure}
    \centering
    ...
    \caption{Three-day sequence
      ...
      algorithm, LAUV temperature data acQuisition and subsequent
...
executed by XP2. The resulting in CUT IN SITU situ temperature data were later
...
An analogous setup was used for SCENARIO OR CASE? scenario D, in which the difference
...
\item D – forecast for 31 October based on CMS data available as prior
  until 29 October;
    \item D1–D4 – corresponding to ERASE A FEW WORDS  assimilation
      configurations as C1–C4, but using CMS data available prior until 30
      October.
\end{itemize}
