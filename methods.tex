...
variability of ocean properties. \kcomment{This is very
...
predictions will likely be inaccurate.  \kcomment{The following
  commented out, doesn't need to be said, does it? See the
  source.}% Equally, if there is a new
...
\emph{a priori} deterministic ocean models \kcomment{This is also
  confusing. Isn't CMEMS a \underline{single} model, i.e. why the
  plural? And isn't CMEMS a numerical model? If so, a simpler sentence
  structure could be easier to understand.} \cite{CMEMS2017} and
direct \kcomment{What does 'direct' mean?} observations. By computing
the pointwise standard deviation across the ensemble, we obtain
spatial uncertainty maps that highlight regions where predictions are
more variable and therefore potentially a target for robotic
...
Beyond uncertainty prediction, the \st{proposed framework}
...
assimilation of AUV acquired data. Data collected on a given day is
...
model. \kcomment{I for one am quiet confused here. What is the 3D
  ...
  clear what/where/how CMEMS is used would help.} This provides both a
...
of greatest expected information gain. More details about the model,
...
\kcomment{I do think the above is a critical and important part of the
  ...
  precisely (i.e. not use 'framework' here) will be important.}
...
October, 2025) (Fig. \ref{fig:scheme}). For simplicity and clarity in
this experiment, we focused on one variable, namely temperature
measurements. As a consequence this work is univariate in showing the
impact of assimilated model-driven exploration. We believe that
results in Sec \ref{sec:results} are general enough to apply to other
critical oceanographic variables which would be modeled similarly. The
